...
class Relay(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        a_start = QAction(make_icon("play", color=PALETTE["t2"]), "Запустить relay.py", self)
        a_start.setShortcut("F5")
        a_start.triggered.connect(self._start)
        m_file.addAction(a_start)
        a_stop = QAction(make_icon("stop", color=PALETTE["t2"]), "Остановить", self)
        a_stop.setShortcut("Shift+F5")
        a_stop.triggered.connect(self._stop)
        m_file.addAction(a_stop)
        a_restart = QAction(make_icon("refresh", color=PALETTE["t2"]), "Перезапустить", self)
        ...
        a_clear = QAction(make_icon("trash", color=PALETTE["t2"]), "Очистить лог", self)
        a_clear.triggered.connect(lambda: self.console.clear_log())
        m_edit.addAction(a_clear)

        # View
        m_view = mb.addMenu("&Вид")
        self._a_pause = QAction(make_icon("pause", color=PALETTE["t2"]), "Приостановить автопрокрутку", self)
        ...
        a_slots = QAction(make_icon("inbox", color=PALETTE["t2"]), "Активные слоты @N…", self)
        a_slots.setShortcut("Ctrl+1")
        a_slots.triggered.connect(self._open_slots)
        m_view.addAction(a_slots)

        # Tools
        m_tools = mb.addMenu("&Сервис")
        a_settings = QAction(make_icon("settings", color=PALETTE["t2"]), "Настройки…", self)
        a_settings.setShortcut("Ctrl+,")
        a_settings.triggered.connect(self._open_settings)
        m_tools.addAction(a_settings)
        a_users = QAction(make_icon("users", color=PALETTE["t2"]), "Пользователи…", self)
        a_users.setShortcut("Ctrl+U")
        a_users.triggered.connect(self._open_users)
        m_tools.addAction(a_users)
        a_cats = QAction(make_icon("cat", color=PALETTE["t2"]), "Категории…", self)
        ...
        a_id = QAction("Как узнать свой Telegram ID", self)
        a_id.triggered.connect(self._show_id_help)
        m_help.addAction(a_id)
        a_site = QAction(make_icon("link", color=PALETTE["t2"]), "Сайт проекта…", self)
